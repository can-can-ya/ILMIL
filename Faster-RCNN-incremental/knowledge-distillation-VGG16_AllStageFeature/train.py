...
def train(**kwargs):
    ...
    # 手动调节学习率
    trainer.faster_rcnn.scale_lr(opt.r)
    ...
    for epoch in range(opt.start_epoch, opt.epoch):
        ...
        if len(opt.VOC_BBOX_LABEL_NAMES_test) > len(
                opt.VOC_BBOX_LABEL_NAMES_all):  # 增量学习，着重看增量学习类别的map，所以绘制的曲线和保存权重的依据都以此为基准
            trainer.vis.plot('test_map', np.nanmean(
                eval_result['ap'][:len(opt.VOC_BBOX_LABEL_NAMES_test)][-len(opt.VOC_BBOX_LABEL_NAMES_all):]))
        else:
        ...
        if len(opt.VOC_BBOX_LABEL_NAMES_test) > len(
                opt.VOC_BBOX_LABEL_NAMES_all):  # 增量学习，着重看增量学习类别的map，所以绘制的曲线和保存权重的依据都以此为基准
            if np.nanmean(eval_result['ap'][:len(opt.VOC_BBOX_LABEL_NAMES_test)][
                          -len(opt.VOC_BBOX_LABEL_NAMES_all):]) > best_map:
                best_map = np.nanmean(
                    eval_result['ap'][:len(opt.VOC_BBOX_LABEL_NAMES_test)][-len(opt.VOC_BBOX_LABEL_NAMES_all):])
